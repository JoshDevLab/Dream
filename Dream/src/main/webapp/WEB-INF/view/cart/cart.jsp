	...
	<jsp:include page="../sidebar.jsp"/>
        ...
        <div id="right-content" style="width: 100%;" class="mt-4">
                ...
                <table class="table border" id="tbl_cart" style="width: 100%;">
                    <thead>
	                    ...
	                    <c:forEach var="cartList" items="${requestScope.cartList}" varStatus="status">
	                    		...
	                    		<tr>
	                                <th scope="row" style ='vertical-align : middle; text-align: center;'>${status.count}</th>
	                                <td style ='vertical-align : middle; text-align: center;'>
	                                    <input type="checkbox" class="check_${status.index}" name="prd_check"><%-- 반복문 넣을때  class 에 뒤에 index값 넣어주기 --%>
	                                </td>
	                                <td style ='vertical-align : middle; text-align: center;'>
	                                    <div class="product_name" style="display: flex;">
	                                        ...
	                                        <div class="prd_info" style="margin-left: auto; width: 70%;">
	                                            ...
	                                            <p class="text-muted mb-0" class="size_jaego">옵션: ${cartList.product_size} / 재고: ${cartList.size_cnt}개 이상</p>
	                                        </div>
	                                    </div>
	                                </td>
	                                <td style ='vertical-align : middle; text-align: center;' class="price_td">
	                                <span class="price"><fmt:formatNumber value="${cartList.price}" pattern="#,###"/></span></td>
	                                <td style ='vertical-align : middle; text-align: center;'>
	                                ...
	                                <td style ='vertical-align : middle; text-align: center;'><span class="total_price"><fmt:formatNumber value="${cartList.price*cartList.cart_cnt}" pattern="#,###"/></span></td> <%-- 반복문으로 넣을때 db에서 받아온 수량이랑 가격 곱해서 넣음 --%>
	                                <td style ='vertical-align : middle; text-align: center;'>
            ...
            <div class="mobile_cart" style="width: 100%;"> <%-- 모바일 장바구니 div 시작 --%> <%-- class 이름 맨뒤에 인덱스 번호 추가--%>
	            <c:if test="${not empty requestScope.cartList}">
	                ...
	                <c:forEach var="cartList" items="${requestScope.cartList}" varStatus="status">
	                <div class="cart_list_${status.index} card moblie_cart_list border-left-0 border-right-0 py-2 mb-3"> <!-- 모바일 화면에 장바구니 한개 상품 묶음 div -->
	                    <div class="top_moblie_cart_list" style="display: flex; position: relative;">
	                        <div class="align-top mr-3">
	                            <input type="checkbox" class="check_${status.index}" name="prd_check"/> <!-- 반복문 넣어줄때 class에 index해서 넣어주기 -->
	                        </div>
	                        ...
	                        <div class="card-img-top-font" style="width: 70%; flex-direction: column;">
	                            ...
	                            <p class="text-muted">옵션: ${cartList.product_size} / 재고: ${cartList.size_cnt}개 이상</p>
	                            <div class="prd_count" style="display: flex;">
	                                <button type="button" class="btn btn-outline-secondary btn-sm minus${status.index}" onclick="minus('minus${status.index}')">-</button>
	                                <input type="text" size="1" value="${cartList.cart_cnt}"/> <!-- value 값으로 DB에 있는 수량 가져오기 -->
		...
		<c:if test="${empty requestScope.cartList}">
            ...
            <div id="cart_footer" class="mt-5" style="width: 100%;"> <%-- 최종가격 div 시작 --%>
                <div id="full_price" class="justify-content-lg-center" style="margin: auto; display: flex;">
                    ...
                    <div class="prd_price_div mr-5" style="display: flex; width: 20%;"> 
                        ...
                        <p style="font-weight: bold; margin-left: auto;"><span id="prd_discount_price">0</span>원</p>
                    </div>
                    <div class="mr-5"> = </div>
                    <div class="prd_price_div mr-5" style="display: flex; width: 20%;"> 
            ...
            <div id="payment_div" class="text-center mt-5">
                <button class="btn btn-lg btn-primary" type="submit"><span class="payment_price">0</span>원 결제하기</button> <%--  --%>
            </div>
        </div> <%-- right-content --%>
    </div>
